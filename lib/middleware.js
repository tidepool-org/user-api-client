...
function isServerRequest(request) {
  return !_.isEmpty(request.headers[legacyServiceSecretHeaderKey]);
}

function isBearerToken(request) {
  return !_.isEmpty(request.headers[authorizationHeader]);
}
...
function validateServerRequest(request, serverSecret, cb) {
  var invalidServerRequest = { message: 'invalid tidepool server request' };
  
  if (request.headers[legacyServiceSecretHeaderKey] && request.headers[authorizationHeader]) {
    return cb(false, invalidServerRequest);
  }
  var secret = request.headers[legacyServiceSecretHeaderKey];
  if (_.isEmpty(secret)) {
    return cb(false);
  }
...
exports.checkToken = function (client, requiredScopes) {
  return function (req, res, next) {
    validateServerRequest(req, client.getServerSecret(), function (valid, err){
      if (err) {
         log.warn(err);
      ...
      if (valid) {
        req._tokendata = { userid: legacyServiceSecretHeaderKey };
        return next();
      }
