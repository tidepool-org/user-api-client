...
module.exports = function (config, hostGetter, request, httpClient) {
  if (httpClient == null) {
    httpClient = require('./httpClient.js')(config, request);
  }
    ...
    login: function (username, password, cb) {
      _withApiHost(cb, function (apiHost) {
        httpClient.requestTo(apiHost + '/login')
          .withMethod('POST')
          .withHeader('x-tidepool-userid', username)
          .withHeader('x-tidepool-password', password)
          .whenStatus(
          200,
          function (res, body, callback) {
    ...
    _.extend(
      retVal,
      {
        ...
        getMetaPair: function (userid, cb) {
          ...
          _withApiHostAndServerToken(cb, function (apiHost, serverToken) {
            httpClient.requestTo(apiHost + '/private/' + userid + '/meta')
              .withToken(serverToken)
              .whenStatus(200, parseJsonHandler)
              .withDefaultHandler(giveThemNothingHandler)
              .go(cb);
